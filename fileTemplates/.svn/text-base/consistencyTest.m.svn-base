...
% of branch and bound iterations to 1 (the default) initially to gauge
% how long the total execution will be. Additionally you can specify the
% display level here. Valid values are ['off'|'notify'|'iter'|'all'|'ALL'] 
opt = DCOptions('maxBranchBoundIter',1,'display','iter')

%Call the ConsistTest constructor
testObj = ConsistencyTest(Dset,opt);
