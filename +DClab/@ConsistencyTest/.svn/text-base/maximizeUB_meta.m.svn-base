function [fval xfeas] = maximizeUB_meta(MMCell,d,u,uncCase,FP,domRng)
...
% - min\gamma s.t. el(1+\gamma)+bL \leq S(x) - d \leq u(1+\gamma) +bH
%
...
% Determine metamodel fitting errors.

b = zeros(m,2);
for i1 = 1:m
  b(i1,:) = MMCell{i1}.fitInfo.peakErrorFromOptimization;
end

confun = @(x) fmconfun(x,MMhandles,d,u,b);
objfun = @(x) x(end);
...
function [iconval,econval] = fmconfun(x,MMhandles,d,u,b)
...
for i1 = 1:m
  ...
  iconval(2*i1-1) = -y(i1) + d(i1) + u(i1,1)*(1+x(end)) + b(i1,1);
  iconval(2*i1) = y(i1) - d(i1) - u(i1,2)*(1+x(end)) - b(i1,2);
end
