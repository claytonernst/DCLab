function [linXlinYnqcqp,logXlinYnqcqp,activeIdxCell] = optim1norm_nqcqp(PD,opt,trgWeights,approxType,nodes)
% function [linXnqcqp,logXnqcqp,activeIdxCell] = optim1norm_nqcqp(PD,top,trgWeights,approxType,nodes)
%
...
% optimization. More info can be found in PolyDataset/oneNormFitX
%
%  Inputs:
...
for i0 = 1:r
  ...
  for i1 = 1:m
    surf = surfaces(PD.PiecewiseSurrogateModelTree,nodes(i0));
  ...
  %build objective function consisting of sum of gammas (the last variables)
  Z0 = zeros(n+m+1);
  Z0(1,1+n+1:1+n+m)=1/2;
  Z0(1+n+1:1+n+m,1)=1/2;
