classdef ResponsePrediction < DClab.DCObject
    ...
    methods
        function obj = ResponsePrediction(varargin)
            
            % Calling syntax we will support
            % ResponsePrediction
            % ResponsePrediction(RM,DSET)    (DSET is type DCDataset or PolyDataset)
            % ResponsePrediction(RM,DSET,OPT)
            % ResponsePrediction(RM,SMTREE,DSET)
            % ResponsePrediction(RM,SMTREE,DSET,[])
            % ResponsePrediction(RM,SMTREE,DSET,OPT)
            % ResponsePrediction(PRED)
            % ResponsePrediction(PRED,OPT)
            
            % We will warn if RM, PRED, or DSET are empty.
            
            % Our strategy is to massage the input list so that it looks like
            % ResponsePrediction(RM,[],DSET,OPT)
            % ResponsePrediction(RM,SMTREE,DSET,OPT) or
            % ResponsePrediction(PRED,OPT).
            ...
            % Massage the inputs
            if isa(varargin{1},'DClab.ResponsePrediction')
                warm = true;
                if ni == 1
                    varargin{2} = DCOptions; %fill in default
                elseif ni == 2
                    assert(isa(varargin{2},'DClab.DCOptions'),'Inputs: 2nd input to RESPONSEPREDICTION of improper class, expecting DCOptions.');
                else
                    error('Inputs: when 1st is a ResponsePrediction object, at most two inputs are allowed.');
                end
            elseif isa(varargin{1},'DClab.ResponseModel')
                warm = false;

                if isa(varargin{2},'DClab.PiecewiseSurrogateModelTree')
                    assert(ni > 2,'Inputs: when 2nd input is a PiecewiseSurrogateModelTree, only 3 or 4 inputs are allowed.');
                    assert(isa(varargin{3},'DClab.DCDataset') && isa(varargin{3},'DClab.PolyDataset'),'Inputs: when 2nd input is a PiecewiseSurrogateModelTree, the 3rd must be a DCDataset or PolyDataset.');
                    if ni == 3
                        varargin = [varargin, {DCOptions}];
                    else
                        assert(isa(varargin{4},'DClab.DCOptions'),'Inputs: expecting a DCOptions object as the 4th input.');
                    end
                    % Since PolyDataset inherits from DCDataset, we only need to check isa Dataset
                elseif isa(varargin{2},'DClab.DCDataset')
                    if ni == 2
                        varargin = [varargin(1), {PiecewiseSurrogateModelTree}, varargin(2), {DCOptions}];
                    elseif ni == 3
                        assert(isa(varargin{3},'DClab.DCOptions'),'Inputs: expecting a DCOptions object as the 3rd input.');
                        varargin = [varargin(1), {PiecewiseSurrogateModelTree}, varargin(2), varargin(3)];
                    else
                        error('Inputs: when 2nd input is a DCDataset or PolyDataset, only 2 or 3 inputs are allowed.');
                    end
                else
                    error('Inputs: when 1st input is a ResponseModel, 2nd must be a PiecewiseSurrogateModelTree, Dataset, or PolyDataset.');
                end
            else
                error('Inputs: 1st input to RESPONSEPREDICTION of improper class');
            end
            ...
            if ~warm
                ...
                if opt.omitOuterBound && opt.maxBranchBoundIter<=1
                    UBo = NaN;
                    LBo = NaN;
                    UBi = max(iterStruct.upper.inner.bnd);
                    LBi = min(iterStruct.lower.inner.bnd);
                elseif opt.omitInnerBound && opt.maxBranchBoundIter<=1
                    UBo = max(iterStruct.upper.outer.bnd);
                    LBo = min(iterStruct.lower.outer.bnd);
                    UBi = NaN;
                    LBi = NaN;
                else
                    [UBo UBoIdx] = max(iterStruct.upper.outer.bnd);
                    [LBo LBoIdx] = min(iterStruct.lower.outer.bnd);
                    UBi = max(iterStruct.upper.inner.bnd);
                    LBi = min(iterStruct.lower.inner.bnd);
                    if UBo-UBi > LBi - LBo
                        iterStruct.worstOBleaf = iterStruct.leafNodes(UBoIdx);
                    else
                        iterStruct.worstOBleaf = iterStruct.leafNodes(LBoIdx);
                    end
                end
                if UBo<UBi
                    UBo = UBi;
                ...
                % condition.
                if UBo-UBi <= opt.branchBoundTermTol && LBi-LBo <= opt.branchBoundTermTol
                    term = true;
                ...
                if opt.maxBranchBoundIter > length(iterStruct)
                    if isnan(LBi)
                        ...
                        if ismember(opt.display,{'iter';'all';'ALL'}) && opt.omitInnerBound
                            ...
                            dcdispstr(str,guihand,false)
                        end
                        try
                            ilb = ResponsePrediction.minimizeUB(iterStruct(end).PSMTree4RM,iterStruct(end).RM,iterStruct(end).PDset,'min',opt,iterStruct(end).leafNodes);
                            iub = ResponsePrediction.minimizeUB(iterStruct(end).PSMTree4RM,iterStruct(end).RM,iterStruct(end).PDset,'max',opt,iterStruct(end).leafNodes);
                        catch ME
                            str = 'Error during warm start one-time inner bound calculation.';
                            dcdispstr(str,guihand,false)
                            dcdispstr(ME,guihand,true)
                        end
                        for i2 = 1:length(iub.pound.bnd)
                            iub.pound.bnd(i2) = -iub.pound.bnd(i2);
                        end
                        iterStruct(end).upper.inner.bnd = iub.pound.bnd;
                        iterStruct(end).upper.inner.xfeas = iub.pound.xfeas;
                        
                        iterStruct(end).lower.inner.bnd = ilb.pound.bnd;
                        iterStruct(end).lower.inner.xfeas = ilb.pound.xfeas;
                        UBi = max(iterStruct(end).upper.inner.bnd);
                        LBi = min(iterStruct(end).lower.inner.bnd);
                    end
                    if isnan(LBo)
                        ...
                        try
                            olb = ResponsePrediction.minimizeLB(iterStruct(end).PSMTree4RM,iterStruct(end).PDset,'min',opt,iterStruct(end).leafNodes,false);
                            oub = ResponsePrediction.minimizeLB(iterStruct(end).PSMTree4RM,iterStruct(end).PDset,'max',opt,iterStruct(end).leafNodes,false);
                        catch ME
                        ...
                        iterStruct(end).upper.outer.bnd = oub.star.bnd;
                        iterStruct(end).upper.outer.pairMult = oub.star.pairMult;
                        iterStruct(end).upper.outer.paramMult = oub.star.paramMult;
                        iterStruct(end).upper.outer.Q = oub.star.Q;
                        iterStruct(end).upper.outer.api = oub.star.api;
                        
                        iterStruct(end).lower.outer.bnd = olb.star.bnd;
                        iterStruct(end).lower.outer.pairMult = olb.star.pairMult;
                        iterStruct(end).lower.outer.paramMult = olb.star.paramMult;
                        iterStruct(end).lower.outer.Q = olb.star.Q;
                        iterStruct(end).lower.outer.api = olb.star.api;
                        UBo = max(iterStruct(end).upper.outer.bnd);
                        LBo = min(iterStruct(end).lower.outer.bnd);
                    end
                end
                ...
                elseif isnan(UBo) || isnan(UBi) || isnan(LBi) || isnan(LBo)
                    term = true;
                    weirdexit = true;
                    exitmsg = 'Returning original object: both inner and outer bounds were not computed in the previous iteration';
                elseif UBo-UBi <= opt.branchBoundTermTol && LBi-LBo <= opt.branchBoundTermTol
                    term = true;
                    weirdexit = true;
            ...
            while ~term
                ...
                iterStruct(i1).RM = iterStruct(i1-1).RM;
                
                [bndU Uidx] = max(iterStruct(i1-1).upper.outer.bnd);
                [bndL Lidx] = min(iterStruct(i1-1).lower.outer.bnd);
                
                %upper or lower bound is worse?
                if bndU-UBi > LBi-bndL
                    isMax=true;
                    UB = UBo;
                    whichLeaf = Uidx;
                else
                ...
                if isMax
                    Q = iterStruct(i1-1).upper.outer.Q{Uidx};
                    sigma_t = UBi; % shouldn't be used in this case;
                else
                    Q = iterStruct(i1-1).lower.outer.Q{Lidx};
                ...
                if opt.omitOuterBound
                    if isMax
                        [dimSplit,dimSplit] = max(abs(iterStruct(i1-1).upper.inner.xfeas{leaf}));
                    else
                        [dimSplit,dimSplit] = max(abs(iterStruct(i1-1).lower.inner.xfeas{leaf}));
                    end
                    ...
                    inherit1 = zeros(iterStruct(i1-1).PSMTree4RM(leaf).nSurfaces,1);
                    inherit2 = zeros(iterStruct(i1-1).PDset.PiecewiseSurrogateModelTree(leaf).nSurfaces,1);
                else
                    [dimSplit, splitLoc, inherit1 inherit2] = ResponsePrediction.findDimension2Divide(iterStruct(i1-1).PSMTree4RM,leaf,1,iterStruct(i1-1).PDset,UB,sigma_t,Q,isMax);
                end
                %[dim, loc, inherit] =
                ...
                iterStruct(i1,1).PSMTree4RM = subdivideDomain(iterStruct(i1-1).PSMTree4RM,{iterStruct(i1-1).RM},leaf,dimSplit,splitLoc,inherit1,opt);
                iterStruct(i1,1).PDset = subdivideDomain(iterStruct(i1-1).PDset,leaf,dimSplit,splitLoc,inherit2,opt);
                
                iterStruct(i1,1).leafNodes = leafNodes(iterStruct(i1).PSMTree4RM);
                ...
                if ~opt.omitOuterBound
                    try
                        ...
                        olb = ResponsePrediction.minimizeLB(iterStruct(i1).PSMTree4RM,iterStruct(i1).PDset,'min',opt,newLeaves,computeGap);
                        if ismember(opt.display,{'iter';'all';'ALL'})
                            str = '  Computing outer upper bound';
                            dcdispstr(str,guihand,false)
                        end
                        oub = ResponsePrediction.minimizeLB(iterStruct(i1).PSMTree4RM,iterStruct(i1).PDset,'max',opt,newLeaves,computeGap);
                    catch ME
                ...
                if ~opt.omitInnerBound
                    try
                        ...
                        ilb = ResponsePrediction.minimizeUB(iterStruct(i1).PSMTree4RM,iterStruct(i1).RM,iterStruct(i1).PDset,'min',opt,newLeaves,computeGap);
                        if ismember(opt.display,{'iter';'all';'ALL'})
                            str = '  Computing inner upper bound';
                            dcdispstr(str,guihand,false)
                        end
                        iub = ResponsePrediction.minimizeUB(iterStruct(i1).PSMTree4RM,iterStruct(i1).RM,iterStruct(i1).PDset,'max',opt,newLeaves,computeGap);
                    catch ME
                ...
                if ~opt.omitOuterBound
                    iterStruct(i1).upper.outer.bnd(parentIdx) = [];
                    iterStruct(i1).upper.outer.pairMult(parentIdx) = [];
                    iterStruct(i1).upper.outer.paramMult(parentIdx) = [];
                    iterStruct(i1).upper.outer.Q(parentIdx) = [];
                    iterStruct(i1).upper.outer.api(parentIdx) = [];
                    iterStruct(i1).upper.outer.bnd = [iterStruct(i1).upper.outer.bnd; oub.star.bnd];
                    iterStruct(i1).upper.outer.pairMult = [iterStruct(i1).upper.outer.pairMult; oub.star.pairMult];
                    iterStruct(i1).upper.outer.paramMult = [iterStruct(i1).upper.outer.paramMult; oub.star.paramMult];
                    iterStruct(i1).upper.outer.Q = [iterStruct(i1).upper.outer.Q; oub.star.Q];
                    iterStruct(i1).upper.outer.api = [iterStruct(i1).upper.outer.api; oub.star.api];
                    
                    iterStruct(i1,1).lower.outer.bnd(parentIdx) = [];
                    iterStruct(i1,1).lower.outer.pairMult(parentIdx) = [];
                    iterStruct(i1,1).lower.outer.paramMult(parentIdx) = [];
                    iterStruct(i1,1).lower.outer.Q(parentIdx) = [];
                    iterStruct(i1,1).lower.outer.api(parentIdx) = [];
                    iterStruct(i1).lower.outer.bnd = [iterStruct(i1).lower.outer.bnd; olb.star.bnd];
                    iterStruct(i1).lower.outer.pairMult = [iterStruct(i1).lower.outer.pairMult; olb.star.pairMult];
                    iterStruct(i1).lower.outer.paramMult = [iterStruct(i1).lower.outer.paramMult; olb.star.paramMult];
                    iterStruct(i1).lower.outer.Q = [iterStruct(i1).lower.outer.Q; olb.star.Q];
                    iterStruct(i1).lower.outer.api = [iterStruct(i1).lower.outer.api; olb.star.api];
                else
                    iterStruct(i1).upper.outer.bnd = [iterStruct(i1).upper.outer.bnd; iterStruct(i1-1).upper.outer.bnd(end)];
                    iterStruct(i1).upper.outer.pairMult = [iterStruct(i1).upper.outer.pairMult; iterStruct(i1-1).upper.outer.pairMult(end)];
                    iterStruct(i1).upper.outer.paramMult = [iterStruct(i1).upper.outer.paramMult; iterStruct(i1-1).upper.outer.paramMult(end)];
                    iterStruct(i1).upper.outer.Q = [iterStruct(i1).upper.outer.Q; iterStruct(i1-1).upper.outer.Q(end)];
                    iterStruct(i1).upper.outer.api = [iterStruct(i1).upper.outer.api; iterStruct(i1-1).upper.outer.api(end)];
                    
                    iterStruct(i1).lower.outer.bnd = [iterStruct(i1).lower.outer.bnd; iterStruct(i1-1).lower.outer.bnd(end)];
                    iterStruct(i1).lower.outer.pairMult = [iterStruct(i1).lower.outer.pairMult; iterStruct(i1-1).lower.outer.pairMult(end)];
                    iterStruct(i1).lower.outer.paramMult = [iterStruct(i1).lower.outer.paramMult; iterStruct(i1-1).lower.outer.paramMult(end)];
                    iterStruct(i1).lower.outer.Q = [iterStruct(i1).lower.outer.Q; iterStruct(i1-1).lower.outer.Q(end)];
                    iterStruct(i1).lower.outer.api = [iterStruct(i1).lower.outer.api; iterStruct(i1-1).lower.outer.api(end)];
                end
                
                if ~opt.omitInnerBound
                    iterStruct(i1).upper.inner.bnd(parentIdx) = [];
                    iterStruct(i1).upper.inner.xfeas(parentIdx) = [];
                    iterStruct(i1).upper.inner.bnd = [iterStruct(i1).upper.inner.bnd; iub.pound.bnd];
                    iterStruct(i1).upper.inner.xfeas = [iterStruct(i1).upper.inner.xfeas; iub.pound.xfeas];
                    
                    iterStruct(i1).lower.inner.bnd(parentIdx) = [];
                    iterStruct(i1).lower.inner.xfeas(parentIdx) = [];
                    iterStruct(i1).lower.inner.bnd = [iterStruct(i1).lower.inner.bnd; ilb.pound.bnd];
                    iterStruct(i1).lower.inner.xfeas = [iterStruct(i1).lower.inner.xfeas; ilb.pound.xfeas];
                else
                    iterStruct(i1).upper.inner.bnd = [iterStruct(i1).upper.inner.bnd; iterStruct(i1-1).upper.inner.bnd(end)];
                    iterStruct(i1).upper.inner.xfeas = [iterStruct(i1).upper.inner.xfeas; iterStruct(i1-1).upper.inner.xfeas(end)];
                    
                    iterStruct(i1).lower.inner.bnd = [iterStruct(i1).lower.inner.bnd; iterStruct(i1-1).lower.inner.bnd];
                    iterStruct(i1).lower.inner.xfeas = [iterStruct(i1).lower.inner.xfeas; iterStruct(i1-1).lower.inner.xfeas(end)];
                end
                
                %if computeGap
                %    iterStruct(i1).upper.outer.qbnd(parentIdx) = [];
                %    iterStruct(i1,1).lower.outer.qbnd(parentIdx) = [];
                %    iterStruct(i1).upper.outer.qbnd = [iterStruct(i1).upper.outer.qbnd; oub.pound.bnd];
                %    iterStruct(i1).lower.outer.qbnd = [iterStruct(i1).lower.outer.qbnd; olb.pound.bnd];
                %end
                
                UBi = max(iterStruct(i1).upper.inner.bnd);
                LBi = min(iterStruct(i1).lower.inner.bnd);
                
                [UBo UBoIdx] = max(iterStruct(i1).upper.outer.bnd);
                [LBo LBoIdx] = min(iterStruct(i1).lower.outer.bnd);
                if UBo<UBi
                    UBo = UBi;
                end
                if LBo>LBi
                ...
                if UBo-UBi > LBi - LBo
                    iterStruct(i1).worstOBleaf = iterStruct(i1).leafNodes(UBoIdx);
                else
                    iterStruct(i1).worstOBleaf = iterStruct(i1).leafNodes(LBoIdx);
                end
            ...
            if strcmp(opt.display,'notify') && weirdexit
                dcdispstr(exitmsg,guihand,false)
        ...
        function [list,sz] = displayProps(obj)
            ...
            if len==0
                ...
                list{1} = sprintf('Outer bounds: [%0.4g, %0.4g]',obj.LBo,obj.UBo);
                list{2} = sprintf('Inner bounds: [%0.4g, %0.4g]',obj.LBi,obj.UBi);
                sz = sprintf('%d iteration',len);
            end
        end
        
        
        function out = get.UBo(obj)
            if isempty(obj.iter(end).upper.outer)
                UBo = nan;
            else
                UBo = max(obj.iter(end).upper.outer.bnd);
            end
            out = UBo;
        end
        function out = get.LBo(obj)
            if isempty(obj.iter(end).lower.outer)
                LBo = nan;
            else
                LBo = min(obj.iter(end).lower.outer.bnd);
            end
            out = LBo;
        end
        function out = get.UBi(obj)
            if isempty(obj.iter(end).upper.inner)
                UBi = nan;
            else
                UBi = max(obj.iter(end).upper.inner.bnd);
            end
            out = UBi;
        end
        function out = get.LBi(obj)
            if isempty(obj.iter(end).lower.inner)
                LBi = nan;
            else
                LBi = min(obj.iter(end).lower.inner.bnd);
            end
            out = LBi;
        end
        function out = get.UBx(obj)
            opt = obj.iter(end).optimOpts;
            if opt.omitInnerBound
                out= {};
            else
                [UB UBiidx] = max(obj.iter(end).upper.inner.bnd);
                out = obj.iter(end).upper.inner.xfeas{UBiidx};
            end
        end
        function out = get.LBx(obj)
            ...
            if opt.omitInnerBound
                ...
                [LB LBiidx] = min(obj.iter(end).lower.inner.bnd);
                out = obj.iter(end).lower.inner.xfeas{LBiidx};
            end
        end
        function out = get.outerBndMults(obj)
            ...
            if opt.omitOuterBound
                ...
                [UB UBoidx] = max(obj.iter(end).upper.outer.bnd);
                [LB LBoidx] = min(obj.iter(end).lower.outer.bnd);
                
                out.upper.expl = obj.iter(end).upper.outer.pairMult{UBoidx}(:,1);
                out.upper.expu = obj.iter(end).upper.outer.pairMult{UBoidx}(:,2);
                out.lower.expl = obj.iter(end).lower.outer.pairMult{LBoidx}(:,1);
                out.lower.expu = obj.iter(end).lower.outer.pairMult{LBoidx}(:,2);
                
                
                upper.tmpl = obj.iter(end).upper.outer.paramMult{UBoidx}(:,1);
                upper.tmpu = obj.iter(end).upper.outer.paramMult{UBoidx}(:,2);
                lower.tmpl = obj.iter(end).lower.outer.paramMult{LBoidx}(:,1);
                lower.tmpu = obj.iter(end).lower.outer.paramMult{LBoidx}(:,2);
                ...
                out.upper.paraml = upper.paraml;
                out.upper.paramu = upper.paramu;
                out.lower.paraml = lower.paraml;
                out.lower.paramu = lower.paramu;
        ...
        function out = get.outerBndSens(obj)
            ...
            if isempty(mults)
                ...
                [UBoidx UBoidx] = max(obj.iter(end).upper.outer.bnd);
                [LBoidx LBoidx] = min(obj.iter(end).lower.outer.bnd);
                apiUB = obj.iter(end).upper.outer.api{UBoidx};
                apiLB = obj.iter(end).lower.outer.api{LBoidx};
                n = obj.iter(end).PDset.nParameters;
                if length(mults.upper.paramu)>n
                    Qu = zeros(2*n+1);
                    Ql = Qu;
                else
                    Qu = zeros(n+1);
                ...
                Qu([1;apiUB],[1;apiUB]) = obj.iter(end).upper.outer.Q{UBoidx};
                Ql([1;apiLB],[1;apiLB]) = obj.iter(end).lower.outer.Q{LBoidx};
                ...
                out.lower.expl = mults.lower.expl;
                out.lower.expu = -mults.lower.expu;
                out.upper.expl = -mults.upper.expl;
                out.upper.expu = mults.upper.expu;
                
                %parameter sensitivity
                out.lower.paraml = -mults.lower.paraml;
                out.lower.paramu = -mults.lower.paramu;
                out.upper.paraml = mults.upper.paraml;
                out.upper.paramu = mults.upper.paramu;
                if size(rng,1)==n
                    for i=1:size(rng,1)
                        if abs(worstRangeL(i,1)-rng(i,1)) < 10*eps
                            out.lower.paraml(i) = -mults.lower.paraml(i).*(Ql(1+i,1)-1.1);
                        else
                            out.lower.paraml(i) = 0;
                        end
                        if abs(worstRangeL(i,2)-rng(i,2)) < 10*eps
                            out.lower.paramu(i) = -mults.lower.paramu(i).*(Ql(1+i,1)+1.1);
                        else
                            out.lower.paramu(i) = 0;
                        end
                        if abs(worstRangeU(i,1)-rng(i,1)) < 10*eps
                            out.upper.paraml(i) = mults.upper.paraml(i).*(Qu(1+i,1)-1.1);
                        else
                            out.upper.paraml(i) = 0;
                        end
                        if abs(worstRangeU(i,2)-rng(i,2)) < 10*eps
                            out.upper.paramu(i) = mults.upper.paramu(i).*(Qu(1+i,1)+1.1);
                        else
                            out.upper.paramu(i) = 0;
                        end
                    end
                else
                    for i=1:size(rng,1)
                        if abs(worstRangeL(i,1)-rng(i,1)) < 10*eps
                            out.lower.paraml(i) = -mults.lower.paraml(i).*(0.5*(Ql(1+i,1)+Ql(1+i+n,1))-1.1);
                        else
                            out.lower.paraml(i) = 0;
                        end
                        if abs(worstRangeL(i,2)-rng(i,2)) < 10*eps
                            out.lower.paramu(i) = -mults.lower.paramu(i).*(0.5*(Ql(1+i,1)+Ql(1+i+n,1))+1.1);
                        else
                            out.lower.paramu(i) = 0;
                        end
                        if abs(worstRangeU(i,1)-rng(i,1)) < 10*eps
                            out.upper.paraml(i) = mults.upper.paraml(i).*(0.5*(Qu(1+i,1)+Qu(1+i+n,1))-1.1);
                        else
                            out.upper.paraml(i) = 0;
                        end
                        if abs(worstRangeU(i,2)-rng(i,2)) < 10*eps
                            out.upper.paramu(i) = mults.upper.paramu(i).*(0.5*(Qu(1+i,1)+Qu(1+i+n,1))+1.1);
                        else
                            out.upper.paramu(i) = 0;
                        end
                    end
                ...
                out.upper.paramu = max(out.upper.paramu.*2./diff(rng,[],2),0);
                out.upper.paraml = min(out.upper.paraml.*2./diff(rng,[],2),0);
                out.lower.paramu = min(out.lower.paramu.*2./diff(rng,[],2),0);
                out.lower.paraml = max(out.lower.paraml.*2./diff(rng,[],2),0);
            end
        end
        function out = get.nIter(obj)
            out = length(obj.iter);
        end
        function out = get.PredictedResponseModelName(obj)
            out = obj.iter(end).RM.name;
        end
        ...
        function out = get.Dataset(obj)
            ...
            FreeParams = obj.iter(1).PDset.FreeParameter;
    ...
    methods (Access=private, Static)
        ...
        function [dim, loc, inherit1, inherit2] = findDimension2Divide(PSMTree,leaf,surfIdx,PDset,UB,qbnd,Q,isMax)
            ...
            [optProb activeIdx] = DClab.ResponsePrediction.makeNqcqp(PSMTree,leaf,surfIdx,PDset,'outer');
        ...
        function [s1 s2] = minimizeLB(PSMTree,PD,extrema,opt,nodes,computeGap,startPt)
            ...
            for i1 = 1:nNodes
                ...
                for i2 = 1:nObjSurfs
                    [optProb activeIdx] = ResponsePrediction.makeNqcqp(PSMTree,nodes(i1),i2,PD,'outer');
                    ...
                    if computeGap
                        ...
                        for i3 = 1:nTries
                            ...
                            if isempty(startPt) || isempty(startPt{i1})
                                if i3 == 1
                                    xinit = findStartPt(PD,nodes(i1),activeIdx,[],Q);
                                else
                                    xinit = findStartPt(PD,nodes(i1),activeIdx,[],[]);
                                end
                            else
                                if i3 == 1
                                    xinit = findStartPt(PD,nodes(i1),activeIdx,startPt{i1},Q);
                                elseif i3 == 2
                                    xinit = findStartPt(PD,nodes(i1),activeIdx,[],Q);
                                else
                                    xinit = findStartPt(PD,nodes(i1),activeIdx,[],[]);
                            ...
                            if strcmpi(opt.display,'all')
                                ...
                                dcdispstr(str,opt.guiHandle,false)
                            else
                                drawnow
                            end
                            ...
                            s2.pound.xfeas{i1}{i2,i3} =  transformAndAddNominals(xfeas,PD,activeIdx,nodes(i1));
            ...
            %       s2.(t{i1}).pound.xfeas(:,i2) =  transformAndAddNominals(xfeas,PD,activeIdx,nodeIndices,t{i1});
        ...
        function [s1 s2] = minimizeUB(PSMTree,RM,PD,extrema,opt,nodes,computeGap,startPt)
            ...
            for i1 = 1:nNodes
                ...
                for i2 = 1:nObjSurfs
                    ...
                    [optProb.m1 activeIdx] = ResponsePrediction.makeNqcqp(PSMTree,nodes(i1),i2,PD,'inner');
                    optProb.m2 = ResponsePrediction.makeNqcqp(PSMTree,nodes(i1),i2,PD,'crude');
                    ...
                    for i3 = 1:nTries
                        ...
                        xfeas =  transformAndAddNominals(xfeas,PD,activeIdx,nodes(i1));
                    ...
                    if ~isequal(optProb.m1,optProb.m2)
                        ...
                        for i3 = 1:N
                            xfeas{i3} = transformAndAddNominals(xfeas{i3},PD,activeIdx,nodes(i1));
                            if isempty(xfeas{i3})
                                feasBool = false;
                            else
...
function iter = BBinitialization(RM,PSMTree4RM,Dset,opt,guihand,dbSaveName)

import DClab.*

%Initialize the fields of iter in the desired order.
fn = {'optimOpts','RM','PSMTree4RM','PDset','worstOBleaf','dimSplit',...
    'splitLoc','leafNodes','upper','lower','runtime'};
iter = cell2struct(cell(11,1),fn);

fn = {'outer','inner'};
iter.upper = cell2struct(cell(2,1),fn);
fn = {'inner','outer'};
iter.lower = cell2struct(cell(2,1),fn);

% Set isPredictor to true so we use a larger safty factor for the fitting
% error.
RM.isPredictor = true;

%Define initial PSMTree4RM
% Make sure sufficient free parameters are available
modelParams = RM.parameterList;
if ~isempty(setdiff(modelParams,Dset.parameterList))
    str = 'Inputs: RM depends on one or more parameters not present in DSET.';
    dcdispstr(str,guihand,true) %display error
end

if isempty(PSMTree4RM)
    if ismember(opt.display,{'iter';'all';'ALL'})
        str = '=====Creating initial PiecewiseSurrogateModelTree from the ResponseModel=====';
        dcdispstr(str,guihand,false)
    end
    
    %construct the surrogateTree
    paramList = {Dset.FreeParameter.name}';
    rng= vertcat(Dset.FreeParameter.range);
    domain = createDomainStructure(paramList,rng);
    
    % TODO, make these inputs;
    critRng = [-inf inf];
    trnRng = [-inf inf];
    
    PSMTree4RM = PiecewiseSurrogateModelTree({RM},domain,domain,critRng,trnRng,opt);
    
else
    % do nothing, PSMTree4RM exists
end
...
iter.leafNodes = leafNodes(PSMTree4RM);
...
if ~opt.omitOuterBound || opt.maxBranchBoundIter>1
    ...
    try
        ...
        %end
        if ismember(opt.display,{'iter';'all';'ALL'}) && opt.omitOuterBound
            str = '  Computing one-time outer lower bound for branching purposes';
            dcdispstr(str,guihand,false)
        elseif ismember(opt.display,{'iter';'all';'ALL'})
            str = '  Computing outer lower bound';
            dcdispstr(str,guihand,false)
        end
        olb = ResponsePrediction.minimizeLB(PSMTree4RM,PDset,'min',opt,iter.leafNodes,computeGap);
        if ismember(opt.display,{'iter';'all';'ALL'}) && opt.omitOuterBound
            str = '  Computing one-time outer upper bound for branching purposes';
            dcdispstr(str,guihand,false)
        elseif ismember(opt.display,{'iter';'all';'ALL'})
            str = '  Computing outer upper bound';
            dcdispstr(str,guihand,false)
        end
        oub = ResponsePrediction.minimizeLB(PSMTree4RM,PDset,'max',opt,iter.leafNodes,computeGap);
    catch ME
    ...
    iter.upper.outer.bnd = oub.star.bnd;
    iter.upper.outer.pairMult = oub.star.pairMult;
    iter.upper.outer.paramMult = oub.star.paramMult;
    iter.upper.outer.Q = oub.star.Q;
    iter.upper.outer.api = oub.star.api;
    
    iter.lower.outer.bnd = olb.star.bnd;
    iter.lower.outer.pairMult = olb.star.pairMult;
    iter.lower.outer.paramMult = olb.star.paramMult;
    iter.lower.outer.Q = olb.star.Q;
    iter.lower.outer.api = olb.star.api;
    
    %if computeGap
    %    iter.upper.outer.qbnd = oub.pound.bnd;
    %    iter.lower.outer.qbnd = olb.pound.bnd;
    %else
        iter.upper.outer.qbnd = [];
        iter.lower.outer.qbnd = [];
    %end
...
if ~opt.omitInnerBound || opt.maxBranchBoundIter>1
    ...
    try
        if ismember(opt.display,{'iter';'all';'ALL'}) && opt.omitInnerBound
            str = '  Computing one-time inner lower bound for branching purposes';
            dcdispstr(str,guihand,false)
        elseif ismember(opt.display,{'iter';'all';'ALL'})
            str = '  Computing inner lower bound';
            dcdispstr(str,guihand,false)
        end
        ilb = ResponsePrediction.minimizeUB(PSMTree4RM,RM,PDset,'min',opt,iter.leafNodes);
        if ismember(opt.display,{'iter';'all';'ALL'}) && opt.omitInnerBound
            str = '  Computing one-time inner upper bound for branching purposes';
            dcdispstr(str,guihand,false)
        elseif ismember(opt.display,{'iter';'all';'ALL'})
            str = '  Computing inner upper bound';
            dcdispstr(str,guihand,false)
        end
        iub = ResponsePrediction.minimizeUB(PSMTree4RM,RM,PDset,'max',opt,iter.leafNodes);
    catch ME
        str = 'call to minimizeUB failed in inner bound computation of ResponsePrediction';
        dcdispstr(str,guihand,false)
        dcdispstr(ME,guihand,true) %display error
    end
    % Correct the upper inner bound for minimizing a negative: This is less
    % complicated than the outer bound case because the bound came from
    % evaluating the true model.
    ...
    iter.upper.inner.bnd = iub.pound.bnd;
    iter.upper.inner.xfeas = iub.pound.xfeas;
    
    iter.lower.inner.bnd = ilb.pound.bnd;
    iter.lower.inner.xfeas = ilb.pound.xfeas;
end
end
