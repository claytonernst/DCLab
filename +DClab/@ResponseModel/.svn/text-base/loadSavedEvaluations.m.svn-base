function [x, y, filesUsed, filesQueried] = ...
    loadSavedEvaluations(RM, domrng, varTrans, Nrequested, filesToSkip)
%LOADSAVEDEVALUATIONS returns saved model evaluations from DClabVxxx/savedEvaluations
%
%   [PARAMVALUES RESPONSE] = LOADSAVEDEVALUATIONS(RM,DOMAINRANGES)
...
%
%   [PARAMVALUES RESPONSE] = LOADSAVEDEVALUATIONS(RM,DOMAINRANGES,VARTRANS)
...
%
%   [PARAMVALUES RESPONSE] = LOADSAVEDEVALUATIONS(RM,DOMAINRANGES,VARTRANS,N)
%   returns at most N (x,y) pairs. If more than N are available, the first
%   N found in the files are returned.
%
%   [PARAMVALUES RESPONSE] = LOADSAVEDEVALUATIONS(RM,DOMAINRANGES,VARTRANS,N,FILESTOSKIP)
%   does not return any pairs saved in the files indicated by FILESTOSKIP.
%  
%   [PARAMVALUES RESPONSE FILESQUERIED] = LOADSAVEDEVALUATIONS(...)
...
%
%   [PARAMVALUES RESPONSE FILESQUERIED FILESUSED] = LOADSAVEDEVALUATIONS(...)
%   additionally returns a row vector listing the numbers of the files that
  ...
  if isempty(varTrans) || isequal(varTrans,catelog(fileIdx).variableTransformations)
    ...
    try
      ...
      if noTransOK && log10TransOK
        ...
        if catelog(fileIdx).forGrad
            n = size(s.a.paramValues,1);
            loadedy = s.a.responses(:,1:(n+1):end);
            loadedx = s.a.paramValues(:,1:(n+1):end);
        else
