function [x, y, fileCreated] = randomEvalWithSave4Grad(RM, domrng, varTrans, N, spread, nComp)
%RANDOMEVALWITHSAVE returns (x,y) pairs for a given ResponseModel to be
%used for gradient estimates
%
%   [PARAMVALUES RESPONSE] = RANDOMEVALWITHSAVE4GRAD(RM,DOMAINRANGES,VARTRANS,N,SPREAD)
%   evalulates the ResponseModel RM n+1 times at each N locations in the
%   set described by the n-by-2 matrix DOMAINRANGES. The N*(n+1) x-values are
%   generated by a latin hypercube design which emulates a uniform
%   distribution over VARTRANS(DOMAINRANGES). VARTRANS should be a nx1 cell
%   array containing 'none' or 'log10'. For example, if DOMAINRANGES = [1
%   10; -1 1]] and VARTRANS = {'log10';'none'} the x of all returned pairs
%   will be [10^zeta(1),; zeta(2)] where zeta(1) was drawn from a uniform
...
%
%   [PARAMVALUES RESPONSE] = RANDOMEVALWITHSAVE4GRAD(RM,DOMAINRANGES,VARTRANS,N,SPREAD,NCOMP)
...
%
%   [PARAMVALUES RESPONSE FILECREATED] = RANDOMEVALWITHSAVE4GRAD(...)
...
%   SPREAD: Spread of n+1 points for derivatives
...
for i1=1:n+1
    x(doesVaryIdx(noTrans),i1:(n+1):end) = repmat(domrng(doesVaryIdx(noTrans),1),1,N) + scale*tmp(:,noTrans)';
end
scale = diag(log10(domrng(doesVaryIdx(log10Trans),2)./domrng(doesVaryIdx(log10Trans),1)));
for i1=1:n+1
    x(doesVaryIdx(log10Trans),i1:(n+1):end) = 10.^(repmat(log10(domrng(doesVaryIdx(log10Trans),1)),1,N) + scale*tmp(:,log10Trans)');
end
spreadMat = repmat([zeros(n,1) rand(n,n) .* repmat(diff(domrng,[],2),1,n) * spread],1,N);
x = x+spreadMat;
...
%answers should all be real
assert(all(all(real(y)==y)),'ResponseModel %s produced not real results',RM.name);
    ...
    for i1 = 1:size(y,1)
        ...
        if ~exist(dirName,'dir')
            if mkdir(evalPath,name) ~= 1
                disp('Could not create save directory.  Make sure save directory is writable.')
                keyboard
            end
        end
